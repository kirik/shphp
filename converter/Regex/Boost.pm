...
=head2 Regex::Boost::boost2php($regex)

Converts a Boost regular expression to a PHP regular expression.  $regex
is a string representing a Boost regular expression.  The result is a string
representing a PHP regular expression or undef if $regex is invalid.
...
sub boost2php ($) {
  ...
  # search through the tokens for things that don't work in PHP (nothing yet)
  foreach my $token (@$tokens) {
    if (ref $token) {
